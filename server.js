...
// البيانات موجودة في نفس المجلد الرئيسي
...
// تشغيل ملفات HTML والملفات الأخرى من المجلد الرئيسي
app.use(
...
app.get('/api/master-data', (req, res) => {
  res.json(
    readJSON(MASTER, {
      schema: 'assets-pro-master-data-v1',
      assets: []
    })
  );
});
...
app.put('/api/master-data', (req, res) => {
  ...
  if (
    body.schema !== 'assets-pro-master-data-v1' ||
    !Array.isArray(body.assets)
  ) {
    return res
      .status(400)
      .send('Invalid master data payload');
  }
...
app.post('/api/inventory/sync', (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .send('Invalid inventory payload');
  }
...
// أي رابط غير معروف يرجع إلى index.html
app.get('*', (req, res) => {
  res.sendFile(
...
app.listen(
  ...
  () => {
    console.log(
      `Assets Pro running on port ${PORT}`
    );
  }
);
